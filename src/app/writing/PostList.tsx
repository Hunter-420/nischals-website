...
export default function PostList({ posts, categories = [] }: { posts: Post[], categories?: Category[] }) {
  ...
  return (
    <div className="flex flex-col gap-10">
            ...
            <div className="flex flex-wrap gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              {selectedCategories.length > 0 && (
                <button
                  ...
                  className="text-sm px-4 py-1.5 rounded-lg transition-colors border bg-transparent text-red-600 border-red-200 hover:bg-red-50 "
                >
                  Clear
                </button>
              )}
      ...
      {/* Post List */}
      <section className="flex flex-col border-t border-slate-200 ">
        {filteredPosts.length > 0 ? (
          filteredPosts.map((post) => (
            <TechnicalArticleCard key={post._id.toString()} post={post} />
          ))
        ) : (
          <p className="text-black font-normal text-base leading-relaxed italic">No posts found for selected topics.</p>
        )}
      </section>
    </div>
  );
